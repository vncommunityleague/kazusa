...
func (r *repositoryImpl) UpsertFlow(ctx context.Context, state string, flow *oidc.Flow) error {
	return r.d.Rds.Do(ctx, r.d.Rds.B().Set().Key(state).Value(rueidis.JSON(flow)).Build()).Error()
}

func (r *repositoryImpl) GetFlow(ctx context.Context, state string) (*oidc.Flow, error) {
	var flow oidc.Flow
